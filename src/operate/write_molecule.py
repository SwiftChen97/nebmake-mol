...
def center_info():
    ...
    """
    center_list = []
    for state in [os.path.join(config.base_dir, sys.argv[1]), os.path.join(config.base_dir, sys.argv[2])]:
...
def run():
    ...
    write_folder = os.path.join(config.base_dir, 'center_neighbor')
    if not os.path.exists(write_folder):
        os.makedirs(write_folder)
    
    # extract the center of mass of each state
    initial = center_info()[0]
    final = center_info()[1]
